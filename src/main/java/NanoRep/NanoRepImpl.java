...
public class NanoRepImpl {
    ...
    private static NanoRepImpl initSession;
    ...
    public static  void initializeNanorep(Context context, String account, String domain) {
        ...
        sDomain = domain;
        initSession = new NanoRepImpl(null, null);
        initSession.fetchDefaultFAQWithCompletion(null);
    }

    /**
     * This constructor configures a NanoRepImpl API wrapper.
     *
     ...
     */
    public NanoRepImpl(String knowledgeBase, HashMap<String, String> nanoContext) {
    ...
    public void searchText(final String text, final NRSearchCompletion completion) {
        if (mCachedSearches != null && mCachedSearches.get(text) != null) {
            ...
            callAPI(params, new NRConnection.Listener() {
                @Override
                public void response(Object responseParam, NRError error) {
                    if (error != null) {
                        ...
                        NanoRepImpl.this.getCachedSearches().put(text, response);
                ...
                public void response(Object responseParam, NRError error) {
                    if (responseParam != null) {
                        ...
                        if (answers != null) {
                            ...
                            NanoRepImpl.this.getCachedSuggestions().put(text, new NRSuggestions((HashMap)responseParam));
                            completion.suggustions(NanoRepImpl.this.mCachedSuggestions.get(text), null);
                        } else {
    ...
    /**
     * Changes the context of NanoRepImpl API wrapper
     *
     ...
     */
    public void answerWithId(String answerId, Nanorep.OnFAQAnswerFetchedListener completion) {
...
//        getFAQ().faqLike(faqLikeParams, completion);
     ...
     *
     * @param completion Callback which contains the NRConfiguration object in case of successful response and NRError in case of unsuccessful
     */
    public void fetchDefaultFAQWithCompletion(Nanorep.OnConfigurationFetchedListener completion) {
//        getFAQ().fetchDefaultFAQWithCompletion(mKnowledgeBase, completion);
    ...
    public NanoRepFAQ getFAQ() {
        if (mFAQ == null) {
            mFAQ = new NanoRepFAQ(mContext, null);
//            mFAQ.setReferer(mReferer);
            ...
            public void response(Object responseParam, NRError error) {
                if (error != null) {
                    ...
                    timer.postDelayed(new Runnable() {
                        public void run() {
                            NanoRepImpl.this.startKeepAlive();
                        }
                    }, (long) NanoRepImpl.this.mDelay);
                }
            }
        });
    }

    private void callAPI(HashMap<String, String> params, final NRConnection.Listener listener) {
        if (mSessionId != null) {
            ...
            api(map, new NRConnection.Listener() {
                @Override
                public void response(Object responseParam, NRError error) {
                    if (error != null) {
                        listener.response(null, error);
                    } else if (responseParam != null && ((HashMap)responseParam).get(SessionIdKey) != null) {
                        NanoRepImpl.this.mSessionId = (String)((HashMap)responseParam).get(SessionIdKey);
                        NanoRepImpl.this.mDelay = (((Integer)((HashMap)responseParam).get(TimeoutKey)).floatValue() / 2) * 1000;
                        Handler timer = new Handler();
                        timer.postDelayed(new Runnable() {
                            public void run() {
                                NanoRepImpl.this.startKeepAlive();
                        ...
                        ArrayList<Object[]> temp = new ArrayList<Object[]>(NanoRepImpl.this.getWaitingAPICalls());
                        for (Object[] arr: temp) {
                            NanoRepImpl.this.callAPI((HashMap)arr[0], (NRConnection.Listener)arr[1]);
                            NanoRepImpl.this.getWaitingAPICalls().remove(arr);
                        }
                        temp = null;
                        NanoRepImpl.this.mWaitingAPICalls = null;
                    }
                }
            });
        }
    ...
    private void api(HashMap<String, String> params, NRConnection.Listener completion) {
